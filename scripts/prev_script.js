...
async function main() {
  ...
  // Use a safe default IPFS base URI if not provided via .env
  // Examples:
  //   IPFS_BASE_URI=ipfs://your-cid/
  //   IPFS_BASE_URI=https://ipfs.io/ipfs/your-cid/
  const baseURIInput = process.env.IPFS_BASE_URI || "ipfs://eventhub-pro/";
  const baseURI = baseURIInput.endsWith("/") ? baseURIInput : `${baseURIInput}/`;

  // Now using metadataURI directly instead of separate description
  const event1URI = `${baseURI}event-one.json`;
  const event2URI = `${baseURI}event-two.json`;
